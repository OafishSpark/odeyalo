...
def matrix_to_imag(matrix: list):
    ...
    for w in range(width):
        for h in range(height):
            img.putpixel((w, h), list_of_colors[matrix[h // 50][w // 50]])
    return img
...
def window():
    ...
    def get_info():
        try:
            ...
            canvas_1 = Canvas(new_window_1, width=50*cols, height=50*rows, 
            ...
            new_window_2.title("Last iter")
            new_window_2.resizable(0, 0)
            new_window_2.wm_attributes("-topmost", 1)
            canvas_2 = Canvas(new_window_2,width=50*cols, height=50*rows, 
                bg="black", highlightthickness=0)
            canvas_2.pack()
            tkimage1 = ImageTk.PhotoImage(matrix_to_imag(blanket))
            canvas_obj = canvas_2.create_image(
                0, 0, 
                image=tkimage1,
                anchor="nw"
            )
